...
std::string DecompressGzip(const std::string& filename) {
  if (filename == "embedded" || filename == "/network.pb.gz") {
    std::string buffer;
    buffer.resize(30 * 1024 * 1024); // 30 MB
    z_stream strm;
    strm.zalloc = Z_NULL;
    strm.zfree = Z_NULL;
    strm.opaque = Z_NULL;
    ...
    if (inflateInit2(&strm, 15 + 32) != Z_OK) throw Exception("inflateInit2 failed");
    strm.avail_out = buffer.size();
    strm.next_out = (Bytef*)buffer.data();
    if (inflate(&strm, Z_FINISH) != Z_STREAM_END) {
      inflateEnd(&strm); throw Exception("inflate failed");
    }
