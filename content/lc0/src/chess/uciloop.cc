...
namespace {
std::queue<std::string> g_uci_queue;
std::mutex g_uci_mutex;
std::condition_variable g_uci_cv;
}

extern "C" {
  __attribute__((used)) void push_uci_command(const char* cmd) {
    std::lock_guard<std::mutex> lock(g_uci_mutex);
    g_uci_queue.push(cmd);
    g_uci_cv.notify_all();
  }
}
...
void UciLoop::RunLoop() {
  ...
  while (true) {
    {
      std::unique_lock<std::mutex> lock(g_uci_mutex);
      g_uci_cv.wait(lock, []{ return !g_uci_queue.empty(); });
      line = g_uci_queue.front();
      g_uci_queue.pop();
    }
    if (line == "EOF") break;
#else
