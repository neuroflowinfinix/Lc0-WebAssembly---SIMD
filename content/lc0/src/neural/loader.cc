...
std::string DecompressGzip(const std::string& filename) {
  // ---- EMBEDDED NETWORK (injected by builder) ----
  if (filename == "embedded") {
    std::string buf;
    buf.resize(30 * 1024 * 1024);
    z_stream s{};
    s.avail_in  = network_data_len;
    s.next_in   = (Bytef*)network_data;
    if (inflateInit2(&s, 15 + 32) != Z_OK)
      throw Exception("inflateInit2 failed");
    s.avail_out = (uInt)buf.size();
    s.next_out  = (Bytef*)buf.data();
    int r = inflate(&s, Z_FINISH);
    inflateEnd(&s);
    if (r != Z_STREAM_END)
      throw Exception("inflate failed: " + std::to_string(r));
    buf.resize(s.total_out);
    CERR << "Loaded embedded network: " << s.total_out << " bytes";
    return buf;
  }
  // ---- END EMBEDDED NETWORK ----
